#!/bin/bash
# ============================================================
# deploy.sh — SOL RSI Monitor v3  (Ubuntu 22.04 + systemd)
...
echo "========================================="
echo " SOL RSI Monitor v3 — Deploy"
...
# ── 4. Directories ───────────────────────────────────────────
mkdir -p "$INSTALL_DIR/logs" "$INSTALL_DIR/data" "$INSTALL_DIR/public/reports"
chown -R "$DEPLOY_USER":"$DEPLOY_USER" \
  "$INSTALL_DIR/logs" "$INSTALL_DIR/data" "$INSTALL_DIR/public/reports" 2>/dev/null || true
...
printf '[Unit]\n' > "$UNIT_FILE"
printf 'Description=SOL RSI Monitor v3 — Jupiter Trading Bot (Whitelist Mode)\n' >> "$UNIT_FILE"
...
printf 'ReadWritePaths=%s/logs %s/data %s/public/reports\n' \
  "$INSTALL_DIR" "$INSTALL_DIR" "$INSTALL_DIR" >> "$UNIT_FILE"
...
echo " Whitelist : POST http://localhost:${PORT}/api/whitelist"
