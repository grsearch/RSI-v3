...
// ── 白名单模式 ─────────────────────────────────────────────────
//
// 【收录】无门槛，webhook 进来直接加入监控列表（白名单模式）
//   同时支持手动添加：POST /api/whitelist  { address, symbol }
//
// 【买入条件】纯 RSI 策略，不检查 FDV/LP：
...
// ── 强制退出门槛（收录时不检查，只在运行中检查）──────────────
const FDV_EXIT_USD = parseInt(process.env.FDV_EXIT_USD || '15000');
...
class TokenMonitor {
  ...
  async addToken({ address, symbol, network = 'solana', xMentions, holders, top10Pct, devPct }) {
    ...
    logger.info(`[Monitor] ✅ Added (whitelist): ${state.symbol} (${address})`);

    // 异步拉取 meta（不做门槛判断，只用于展示和退出检查）
    this._fetchMeta(state);
  ...
  start() {
    logger.info(
      `[Monitor] Starting (whitelist mode)` +
      ` | exit FDV<$${FDV_EXIT_USD} LP<$${LP_EXIT_USD}` +
      ` | window ${TOKEN_MAX_AGE_MIN}min (${(TOKEN_MAX_AGE_MIN / 60).toFixed(0)}h)` +
      ` | SL ${STOP_LOSS_PCT}%`
    );
  ...
  _stateView(s) {
    ...
    return {
      ...
      timeLeft:     Math.max(0, Math.round((maxMs - (Date.now() - s.addedAt)) / 1000)),
